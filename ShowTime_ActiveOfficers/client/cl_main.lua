...
-- Safety: Force shutdown loading screen/spinner
...
local function loadPlayerSettings()
    Bridge.TriggerCallback('ShowTime_ActiveOfficers:GetMySettings', function(s)
        ...
        -- Final safety check to ensure loading screen is gone
        ShutdownLoadingScreen()
        BusyspinnerOff()
    end)
end
...
-- Initial check in case player is already loaded (script restart)
CreateThread(function()
    Wait(1000)
    local p = Bridge.GetPlayerData()
    if p and p.job then
...
-- Notifications handled by framework bridge
