...
local function DetectFramework()
    ...
    if GetResourceState('qbx_core') == 'started' or GetResourceState('qbx-core') == 'started' then
        local resourceName = GetResourceState('qbx_core') == 'started' and 'qbx_core' or 'qbx-core'
        local success, core = pcall(function() return exports[resourceName]:GetCoreObject() end)
        if success and core then
...
function Bridge.Init()
    local framework = Config.Framework == "auto" and DetectFramework() or Config.Framework
    local voice = Config.VoiceSystem == "auto" and DetectVoiceSystem() or Config.VoiceSystem
    
    if not framework then
        print("^1[ShowTime_ActiveOfficers] ERROR: No supported framework (QBCore/QBox) detected! This script is not standalone.^7")
    end
